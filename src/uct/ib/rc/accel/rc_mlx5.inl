                                    ...
                                    unsigned byte_len, int poll_flags)
{
    ...
    if (ucs_unlikely(hdr->rc_hdr.am_id & UCT_RC_EP_FC_MASK)) {
        ...
        if (poll_flags & UCT_RC_MLX5_POLL_FLAG_SIG) {
            ...
            params.sig = uct_ib_mlx5_calc_sig(base_iface->rx_allocator.cache.memh,
                    payload, byte_len - uct_ib_iface_tl_hdr_length(&iface->super.super));
        }
                             ...
                             uct_ib_log_sge_t *log_sge)
{
    ...
    if (ucs_likely(iface != NULL)) {
        uct_ib_mlx5_log_tx(&iface->super.super,
...
{
    uct_base_iface_t *base_iface   = &iface->super.super.super;
    uct_ib_mlx5_md_t *md = ucs_derived_of(base_iface->md, uct_ib_mlx5_md_t);
    ...
    if (poll_flags & UCT_RC_MLX5_POLL_FLAG_SIG) {
        uct_ib_mlx5_sig_mr_get_data(md, base_iface->rx_allocator.cache.memh,
                                         ...
                                         unsigned poll_flags)
{
    ...
    if (desc_map & UCS_BIT(UCT_IB_RX_SG_PAYLOAD_IDX)) {
        ...
        uct_rc_mlx5_iface_common_seg_set_sge_payload_entry(iface, seg, poll_flags);
    }
                                       ...
                                       unsigned poll_flags)
{
    if (ucs_likely(!UCT_RC_MLX5_MP_ENABLED(iface))) {
        uct_rc_mlx5_iface_srq_post_recv(
                iface, uct_rc_mlx5_iface_common_srq_set_seg_sge, poll_flags);
    } else {
